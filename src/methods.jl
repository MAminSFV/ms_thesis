# REVIEW - What does Methods.jl do?
# does it solve things? what does it solve? let's find out (you get the reference right?)
# It is about solvers and how to update the iterations and check constraints, I still don't fully understand it

# TODO - The only function of concern is the trim_conditions_batch. (I guess it is optional for me now)

# REVIEW - What is prob0 ?
...
function solve_admm_1slack(prob_lift, prob_load, quad_params, load_params, admm_type, opts, max_iters=3, n_slack=3)
	...
	@show quat
    # REVIEW - Why are there so many unused vars?
    # Problem dimensions
    ...
    # SECTION - SOLVE INITIAL PROBLEMS 
    ...
    # NOTE - First lifts are updated then the load.
    for i = 1:num_lift
    ...
    # SECTION - Create augmented Lagrangian problems, solvers
    ...
    # SECTION - SOLVE ADMM 
    # REVIEW - I have no clue what is going on under the hood. Specifically solve_aula!
    for ii = 1:max_iters
        # NOTE - Step 1: Solve lift agents
		@info "Solve quads"
        for i = 1:num_lift

            TO.solve_aula!(prob_lift_al[i],solver_lift_al[i])

            #  NOTE - Update constraints (sequentially)
        ...
        #  NOTE - Update constraints (parallel)
        ...
        # NOTE - Step 2: Solve load
        # return prob_lift,prob_load,1,1
		@info "Solve load"
        prob_load_al.model = gen_load_model(X_lift,N,dt,load_params) # NOTE - Model generated here
        ...
        for i = 1:num_lift
            prob_lift_al[i].model = gen_lift_model(X_load,N,dt,quad_params,quat) # NOTE - Model generated here
        end
...
# SECTION - Lift-Cable and Load Cable Constraints
# REVIEW - What is this doing? I guess it ensures cables are not slacking, cable constraints (c[1], c[2]?)
# NOTE - Pay attention to Lift - Cable constraint
function gen_lift_cable_constraints_1slack(X_load,U_load,agent,n,m,d,n_slack=3)
    N = length(X_load) #what?
    con_cable_lift = []
    Is = Diagonal(I,n_slack)
    # REVIEW - What is being calculated here? Something related to the forces, is this a slack test?
    # What is c[1] and c[2]? They appear to be constraint values
    # is c[1] the cable length for an agent?
    # is c[2] the cable force mismatch? it doesn't make sense.
    for k = 1:N
        function con(c,x,u=zeros())
            if k == 1
                c[1] = u[end] - U_load[k][(agent-1) + 1]
            else
                c[1] = norm(x[1:n_slack] - X_load[k][1:n_slack])^2 - d^2 # That is a distance calculation it seems.
                if k < N
                    c[2] = u[end] - U_load[k][(agent-1) + 1]
                end
            end
        end
        # REVIEW - And what is this?
        function ∇con(C,x,u=zeros())
            x_pos = x[1:n_slack]
...
# REVIEW - Same function but for load, have to pay more attention
# NOTE - Pay attention to Load- Cable Constraint
...
# SECTION - Trim conditions for different scenarios
# REVIEW - What is a trim condition?
...
# NOTE - here, might need to make a trim_conditions_platform_batch
# REVIEW - Honestly I don't know how this helps, probably by pre-solving?
